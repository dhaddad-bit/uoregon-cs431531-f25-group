...
int main(int argc, char** argv) {
	...
	outFile << "csr_cpu," << timer[SPMV_TIME] << std::endl;
    ...
    for (int i = 0; i < num_tests; i++) {
	    ...
	    outFile << "csr_gpu," << timer[GPU_SPMV_TIME] << std::endl; 
        }
    };
    ...
    for (int i = 0; i < num_tests; i++) {
	    ...
	    outFile << "ell_std," << timer[GPU_ELL_TIME] << std::endl;
        }
    }
	...
	outFile << "ell_sig," << timer[GPU_SELL_C_TIME] << std::endl;
