...
__global__ void spmv_csr5_kernel(
    ...
    const P_TYPE* __restrict__ x,
    P_TYPE* __restrict__ y
) {
    int tid = blockIdx.x;
...
extern "C" void spmv_gpu_csr5(
    ...
    cudaMemset(d_y, 0, m *sizeof(P_TYPE));
    ...
    *time_ms = elapsedTime / (float)MAX_ITER;
    ...
    cudaMemset(d_y, 0, m *sizeof(P_TYPE)); // reset output again after timing Final clean run for correctenss
    spmv_csr5_kernel<<<grid, block>>>(num_tiles, d_val, d_col_idx, d_row_idx, d_tile_ptr, d_tile_desc, d_x, d_y);
